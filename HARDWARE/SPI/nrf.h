...
void Nrf24l01_Init(u8 model, u8 ch,u8 sel);				//???,model=1/2/3/4,ch???????
void NRF_TxPacket(uint8_t * tx_buf, uint8_t len,u8 sel);			//?????,??model 2/4
void NRF_TxPacket_AP(uint8_t * tx_buf, uint8_t len,u8 sel);	//?????,??model 3
uint8_t NRF_Read_Reg(uint8_t re,u8 selg);
...
uint8_t Nrf24l01_Check(u8 sel);
extern void SPI_CE_H(u8 sel)  ;
extern void SPI_CE_L(u8 sel)  ;
extern void SPI_CSN_H(u8 sel)  ;
extern void SPI_CSN_L(u8 sel)  ;
#endif
